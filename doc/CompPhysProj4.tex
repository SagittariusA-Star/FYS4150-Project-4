...
\begin{align}
	\langle M(T)\rangle \sim (T - T_C)^\beta \to L^{-\beta/\nu},
\end{align}
the heat capacity becomes 
\begin{align}
	C_V(T) \sim |T_C - T|^{-\alpha}\to L^{\alpha / \nu}
\end{align}
and the susceptibility is 
\begin{align}
	\chi(T)\sim |T_C - T|^{-\gamma} \to L^{\gamma/\nu},
\end{align}
where we have set $T_C(L) = T$ and $T_C(L\to\infty) = T_C$.
Thus if we plot the mean energy and magnetisation 
